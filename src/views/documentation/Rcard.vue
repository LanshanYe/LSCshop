  ...
  <query ref="querycomponent" :list-query="listQuery" :api="api">
    ...
    <el-table-column slot="tableColumn" :label="$t('table.actions')" align="center" width="230" class-name="small-padding fixed-width">
      <template slot-scope="scope">
        <el-button type="primary" size="mini" @click="handleUpdate(scope.row.RDID)">{{ $t('table.edit') }}</el-button>
        <el-button size="mini" type="danger" @click="deleteData(scope.row.RDID)">{{ $t('table.delete') }}
        </el-button>
      </template>
    </el-table-column>
  </query>
